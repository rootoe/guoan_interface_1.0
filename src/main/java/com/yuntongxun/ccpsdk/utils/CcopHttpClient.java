...
public class CcopHttpClient {
	private static final Logger logger = LoggerFactory.getLogger(CcopHttpClient.class);
	...
	public DefaultHttpClient registerSSL(String hostname, String protocol, int port, String scheme) throws NoSuchAlgorithmException, KeyManagementException {
		...
		// 如果云通讯代理开关
		if (Const.YUNTONGXUN_PROXY_SWITCH) {
			logger.info("云通讯代理打开");
			httpclient = CCPProxyUtil.getHttpClient(httpclient);
		}
