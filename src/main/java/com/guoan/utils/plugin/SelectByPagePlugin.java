package com.guoan.utils.plugin;
...
public class SelectByPagePlugin extends PluginAdapter {
	...
	private void addField(String fieldName, FullyQualifiedJavaType fieldType, TopLevelClass topLevelClass) {
	...
	public boolean modelExampleClassGenerated(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
		...
		getSkipCountMethod.addBodyLine("return (this.pageIndex-1)*this.pageSize;");
		...
		getEndRecordIndexMethod.addBodyLine("return this.pageIndex*this.pageSize;");
	...
	public boolean sqlMapDocumentGenerated(Document document, IntrospectedTable introspectedTable) {
		...
		newResultMapElement.addAttribute(new Attribute("id", "selectPageResult"));
		newResultMapElement.addAttribute(new Attribute("extends", "BaseResultMap"));
		...
		newResultMapElement.addElement(new TextElement("<!--\r\n<association property=\"\" column=\"\" javaType=\"\">\r\n" + "   <id column=\"\" property=\"\" jdbcType=\"\" />\r\n"
		...
		// 以下代码用于生成支持分页的sql片段
		/**
		 * 注意千万不要随意删除代码中的空格 ！ do not remove spaces
		 */
		...
		answer.addAttribute(new Attribute("id", //$NON-NLS-1$
				"selectPage"));
		answer.addAttribute(new Attribute("resultMap", "selectPageResult")); //$NON-NLS-1$
